...
export interface DesktopLoginCompleteResponse {
...
export async function completeDesktopLogin(sessionToken: string): Promise<DesktopLoginCompleteResponse> {
  const { data } = await apiClient.post<DesktopLoginCompleteResponse>('/auth/desktop/complete', {
