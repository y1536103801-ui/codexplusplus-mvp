...
| `docker-compose.dev.yml` | Local source build for isolated development and Codex++ route inspection |
| `docker-deploy.sh` | **One-click Docker deployment script (recommended)** |
| `.env.example` | Docker environment variables template |
| `.env.codexplus-local.example` | Example local-source env file; copy to `.env.codexplus-local` and keep real values uncommitted |
...
### Local Source Build Without Replacing 8080

Use `docker-compose.dev.yml` when you need to run the current workspace source beside an existing `weishaw/sub2api:latest` container. The default local-source endpoint is `http://127.0.0.1:8081`, and data is isolated under `.codexplus-local/` so it does not reuse `data/`, `postgres_data/`, or `redis_data/`.

Recommended one-command local start from the repository root:
...
That wrapper creates the ignored `.env.codexplus-local` file when needed, starts `docker-compose.dev.yml` with the fixed `sub2api-codexplus-local` project name, confirms the app container is compose-owned, and then probes the 07 routes.
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File ../tools/e2e/codexplus/start-local-source-service.ps1 -Root (Resolve-Path ..\..) -SkipBuild -ProbeOnly -HostPort 8081
```

If `8081` is already occupied, set another host port in `.env.codexplus-local`:

```dotenv
SUB2API_DEV_HOST_PORT=8082
```

Then use the same port in the probe command:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File ../tools/e2e/codexplus/start-local-source-service.ps1 -Root (Resolve-Path ..\..) -SkipBuild -ProbeOnly -HostPort 8082
```

The local `.env.codexplus-local` file and `.codexplus-local/` data directory are git-ignored. Common dev-stack lifecycle commands:

```powershell
docker compose --env-file .env.codexplus-local -p sub2api-codexplus-local -f docker-compose.dev.yml ps
docker compose --env-file .env.codexplus-local -p sub2api-codexplus-local -f docker-compose.dev.yml logs -f sub2api
docker compose --env-file .env.codexplus-local -p sub2api-codexplus-local -f docker-compose.dev.yml up -d --build
docker compose --env-file .env.codexplus-local -p sub2api-codexplus-local -f docker-compose.dev.yml down
```

Use `down -v` only when you intentionally want to remove the compose-managed local volumes. Remove `.codexplus-local/` only when you intentionally want to delete the isolated local app/postgres/redis data.

`start-local-dev-compose.ps1` is the preferred entry for the fresh isolated local-source stack. `start-local-source-service.ps1 -ProbeOnly` is the route preflight helper for a running local source service. Its non-`ProbeOnly` mode is a compatibility helper that builds/runs a source container against an existing `sub2api` Docker stack; prefer `start-local-dev-compose.ps1` or `docker-compose.dev.yml` for isolated local work.
...
1. **Port already in use**: For `docker-compose.dev.yml`, change `SUB2API_DEV_HOST_PORT` in `.env.codexplus-local`; for production/local compose, change `SERVER_PORT` in `.env` or systemd config
