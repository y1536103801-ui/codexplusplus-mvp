...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	codexPlusEventRepository := repository.NewCodexPlusEventRepository(db)
	authHandler := handler.NewAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, userAttributeService, codexPlusEventRepository)
	...
	codexPlusConfigService := service.NewCodexPlusConfigService(settingRepository)
	codexPlusDeviceRepository := repository.NewCodexPlusDeviceRepository(db)
	codexPlusManagedProviderKeyRepository := repository.NewCodexPlusManagedProviderKeyRepository(db)
	codexPlusGatewayPolicyService := service.ProvideCodexPlusGatewayPolicyService(codexPlusConfigService, codexPlusManagedProviderKeyRepository, codexPlusDeviceRepository, codexPlusEventRepository, billingCacheService)
	codexPlusClientService := service.ProvideCodexPlusClientService(codexPlusConfigService, userService, subscriptionService, apiKeyService, redeemService, codexPlusDeviceRepository, codexPlusManagedProviderKeyRepository, codexPlusEventRepository)
	codexPlusAdminService := service.NewCodexPlusAdminService(codexPlusConfigService, settingRepository, adminService, subscriptionService, paymentConfigService, codexPlusDeviceRepository, codexPlusEventRepository)
	codexPlusHandler := admin.NewCodexPlusHandler(codexPlusAdminService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler, codexPlusHandler)
	...
	gatewayHandler.SetCodexPlusGatewayPolicyService(codexPlusGatewayPolicyService)
	...
	openAIGatewayHandler.SetCodexPlusGatewayPolicyService(codexPlusGatewayPolicyService)
	handlerSettingHandler := handler.ProvideSettingHandler(settingService, buildInfo, notificationEmailService)
	totpHandler := handler.NewTotpHandler(totpService)
	handlerPaymentHandler := handler.NewPaymentHandler(paymentService, paymentConfigService, channelService)
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentService, registry)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	clientHandler := clienthandler.NewClientHandler(codexPlusClientService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, clientHandler, idempotencyCoordinator, idempotencyCleanupService)
