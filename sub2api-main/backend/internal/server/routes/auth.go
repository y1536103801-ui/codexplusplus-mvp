...
func RegisterAuthRoutes(
	...
	auth.Use(servermiddleware.BackendModeAuthGuard(settingService))
	{
		...
		auth.POST("/desktop/start", rateLimiter.LimitWithOptions("auth-desktop-start", 20, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.StartDesktopLogin)
		auth.POST("/desktop/poll", rateLimiter.LimitWithOptions("auth-desktop-poll", 60, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.PollDesktopLogin)
