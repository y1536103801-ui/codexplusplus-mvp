...
func (h *GatewayHandler) SetCodexPlusGatewayPolicyService(policyService *service.CodexPlusGatewayPolicyService) {
	if h != nil {
		h.codexPlusPolicyService = policyService
	}
}
...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if !h.enforceCodexPlusGatewayPolicy(c, apiKey, subscription, reqModel, "/v1/messages", func(status int, code, message string) {
...
func (h *GatewayHandler) CountTokens(c *gin.Context) {
	...
	if !h.enforceCodexPlusGatewayPolicy(c, apiKey, subscription, parsedReq.Model, "/v1/messages/count_tokens", func(status int, code, message string) {
		h.errorResponse(c, status, code, message)
	}) {
		return
	}
