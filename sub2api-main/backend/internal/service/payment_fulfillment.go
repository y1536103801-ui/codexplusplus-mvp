...
func (s *PaymentService) doSub(ctx context.Context, o *dbent.PaymentOrder) error {
	...
	commerce := s.codexPlusCommerceEntitlementService()
	entitlement, err := commerce.ResolveSubscriptionOrder(ctx, o)
	if err != nil {
		return fmt.Errorf("resolve Codex++ entitlement: %w", err)
	}
	// Idempotency: audit logs cover completed orders, while subscription notes cover
	// the retry window after assignment succeeds but before completion is recorded.
	if commerce.AlreadyGranted(ctx, o, entitlement) {
		slog.Info("subscription already assigned for order, skipping", "orderID", o.ID, "groupID", gid)
		commerce.RecordAlreadyGranted(ctx, o, entitlement)
		return s.markCompleted(ctx, o, "SUBSCRIPTION_SUCCESS")
	}
	orderNote := commerce.GrantNote(o, entitlement)
	sub, reused, err := s.subscriptionSvc.AssignOrExtendSubscription(ctx, &AssignSubscriptionInput{UserID: o.UserID, GroupID: gid, ValidityDays: days, AssignedBy: 0, Notes: orderNote})
	if err != nil {
		return fmt.Errorf("assign subscription: %w", err)
	}
	commerce.RecordGrant(ctx, o, entitlement, sub, reused)
	return s.markCompleted(ctx, o, "SUBSCRIPTION_SUCCESS")
}
