...
func ProvideCodexPlusClientService(
	...
	managedKeyRepo CodexPlusManagedProviderKeyRepository,
	eventRepo CodexPlusEventRepository,
) *CodexPlusClientService {
	svc := NewCodexPlusClientService(configService, userService, subscriptionService, apiKeyService, redeemService)
	svc.SetDeviceStore(deviceRepo)
	svc.SetManagedProviderKeyStore(managedKeyRepo)
	svc.SetEventSink(codexPlusClientEventSink{repo: eventRepo})
	return svc
}

func ProvideCodexPlusGatewayPolicyService(
	configService *CodexPlusConfigService,
	managedKeyRepo CodexPlusManagedProviderKeyRepository,
	deviceRepo CodexPlusDeviceRepository,
	eventRepo CodexPlusEventRepository,
	billingCacheService *BillingCacheService,
) *CodexPlusGatewayPolicyService {
	return NewCodexPlusGatewayPolicyService(
		WithCodexPlusGatewayPolicyConfigReader(configService),
		WithCodexPlusManagedProviderKeyReader(managedKeyRepo),
		WithCodexPlusGatewayDeviceReader(deviceRepo),
		WithCodexPlusGatewayPolicyEventRecorder(eventRepo),
		WithCodexPlusGatewayBillingChecker(billingCacheService),
	)
}

type codexPlusClientEventSink struct {
	repo CodexPlusEventRepository
}

func (s codexPlusClientEventSink) Record(ctx context.Context, event CodexPlusClientEvent) error {
	if s.repo == nil {
		return nil
	}
	...
	input := CodexPlusEventCreate{
		EventType:     event.Type,
		Severity:      "info",
		RequestID:     stringPtrIfNotBlank(event.RequestID),
		ConfigVersion: stringPtrIfNotBlank(event.ConfigVersion),
		Payload:       payload,
	}
	if event.UserID > 0 {
		userID := event.UserID
	...
	if trimmed := strings.TrimSpace(event.DeviceID); trimmed != "" {
		input.DeviceID = &trimmed
	}
	_, err := s.repo.Append(ctx, input)
	return err
}
...
var ProviderSet = wire.NewSet(
	...
	ProvideCodexPlusGatewayPolicyService,
	...
	NewCodexPlusConfigService,
	ProvideCodexPlusClientService,
	NewCodexPlusAdminService,
