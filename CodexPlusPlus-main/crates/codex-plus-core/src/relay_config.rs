...
fn relay_profile_base_url(profile: &RelayProfile) -> String {
    if let Some(base_url) = codexplus_managed_upstream_base_url(profile) {
        return base_url;
    }
...
fn complete_relay_profile_config(profile: &RelayProfile) -> anyhow::Result<String> {
    ...
    let provider_base_url = if is_codexplus_managed_profile(profile) {
        crate::protocol_proxy::local_responses_proxy_base_url(
            crate::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT,
        )
    } else {
...
pub fn normalize_relay_profile_for_storage(profile: &mut RelayProfile) -> anyhow::Result<()> {
    ...
    let source_base_url = codexplus_managed_upstream_base_url(profile)
        .unwrap_or_else(|| relay_profile_base_url(profile));
...
fn is_codexplus_managed_profile(profile: &RelayProfile) -> bool {
    profile.id == crate::codexplus_cloud::provider_writer::MANAGED_PROVIDER_ID
}

fn codexplus_managed_upstream_base_url(profile: &RelayProfile) -> Option<String> {
    if !is_codexplus_managed_profile(profile) {
        return None;
    }
    let upstream_base_url = profile.upstream_base_url.trim();
    if !upstream_base_url.is_empty() {
        return Some(upstream_base_url.to_string());
    }
    ...
    if !base_url.is_empty() && base_url != local_proxy_base_url.trim_end_matches('/') {
