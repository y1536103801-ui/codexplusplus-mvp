...
fn relay_protocol_proxy_enabled(settings: &BackendSettings) -> bool {
    ...
    profile.protocol == crate::settings::RelayProtocol::ChatCompletions
...
async fn handle_protocol_proxy_connection(
    ...
    if upstream.is_stream {
        let stream_content_type = if upstream.body_kind
            == crate::protocol_proxy::UpstreamProxyBodyKind::Responses
            && !upstream.content_type.is_empty()
        {
            upstream.content_type.clone()
        } else {
        ...
        if upstream.body_kind == crate::protocol_proxy::UpstreamProxyBodyKind::Responses {
            let mut bytes_stream = upstream.response.bytes_stream();
            while let Some(chunk) = bytes_stream.next().await {
                stream.write_all(&chunk?).await?;
            }
            log_helper_response(
                "helper.protocol_proxy_passthrough_stream_ok",
                method,
                path,
                "200 OK",
                remote_addr_text,
    ...
    if upstream.body_kind == crate::protocol_proxy::UpstreamProxyBodyKind::Responses {
        ...
        log_helper_response(
            "helper.protocol_proxy_passthrough_ok",
            method,
            path,
            "200 OK",
            remote_addr_text,
