...
pub struct UpstreamProxyResponse {
    ...
    pub content_type: String,
    pub is_stream: bool,
    pub body_kind: UpstreamProxyBodyKind,
    pub response: reqwest::Response,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum UpstreamProxyBodyKind {
    ChatCompletions,
    Responses,
}
...
pub async fn open_responses_proxy_request(body: &str) -> anyhow::Result<UpstreamProxyResponse> {
    ...
    if is_codexplus_managed_profile(&relay) {
        return open_codexplus_responses_proxy_request(body, &relay).await;
    }
    if relay.protocol == RelayProtocol::ChatCompletions {
        return open_chat_backed_responses_proxy_request(body, &relay).await;
    }
    anyhow::bail!("当前中转未启用 Responses 协议代理");
}

async fn open_chat_backed_responses_proxy_request(
    body: &str,
    relay: &RelayProfile,
) -> anyhow::Result<UpstreamProxyResponse> {
    let api_key = relay_proxy_api_key_for_profile(relay);
    ...
    Ok(UpstreamProxyResponse {
        ...
        content_type,
        body_kind: UpstreamProxyBodyKind::ChatCompletions,
        response: upstream,
    })
}

async fn open_codexplus_responses_proxy_request(
    body: &str,
    relay: &RelayProfile,
) -> anyhow::Result<UpstreamProxyResponse> {
    let upstream_base_url = codexplus_upstream_base_url(relay);
    let api_key = relay_proxy_api_key_for_profile(relay);
    if upstream_base_url.trim().is_empty() {
        anyhow::bail!("Codex++ Cloud gateway Base URL 不能为空");
    }
    if api_key.trim().is_empty() {
        anyhow::bail!("Codex++ Cloud gateway Key 不能为空");
    }
    ...
    let is_stream = request_json
        ...
        .unwrap_or(false);
    let client = crate::http_client::proxied_client(&relay.user_agent)?;
    let upstream = with_codexplus_device_headers(
        client
            .post(responses_url(&upstream_base_url))
            .bearer_auth(api_key.trim())
            .header(reqwest::header::CONTENT_TYPE, "application/json"),
    )
    .json(&request_json)
    ...
    Ok(UpstreamProxyResponse {
        ...
        is_stream: is_stream || content_type.contains("text/event-stream"),
        content_type,
        body_kind: UpstreamProxyBodyKind::Responses,
        response: upstream,
    })
}
...
pub async fn open_models_proxy_request() -> anyhow::Result<UpstreamProxyResponse> {
    let settings = SettingsStore::default().load().unwrap_or_default();
    let relay = settings.active_relay_profile();
    let codexplus_managed = is_codexplus_managed_profile(&relay);
    if relay.protocol != RelayProtocol::ChatCompletions && !codexplus_managed {
        anyhow::bail!("当前中转未启用模型列表代理");
    }
    let base_url = if codexplus_managed {
        codexplus_upstream_base_url(&relay)
    } else {
    ...
    let client = crate::http_client::proxied_client(&relay.user_agent)?;
    let upstream = with_codexplus_device_headers_if(
        client
...
fn is_codexplus_managed_profile(relay: &RelayProfile) -> bool {
    relay.id == crate::codexplus_cloud::provider_writer::MANAGED_PROVIDER_ID
}

fn codexplus_upstream_base_url(relay: &RelayProfile) -> String {
    relay
        .upstream_base_url
        .trim()
        .to_string()
        .if_empty_then(|| relay.base_url.trim().to_string())
}

pub fn relay_proxy_api_key_for_profile(relay: &RelayProfile) -> String {
    let api_key = relay.api_key.trim();
    if !api_key.is_empty() {
        return api_key.to_string();
    }
    codexplus_proxy_auth_api_key(&relay.auth_contents).unwrap_or_default()
}

fn codexplus_proxy_auth_api_key(auth_contents: &str) -> Option<String> {
...
fn with_codexplus_device_headers(builder: reqwest::RequestBuilder) -> reqwest::RequestBuilder {
    let builder = builder.header("X-CodexPlus-Client-Version", crate::version::VERSION);
    match crate::codexplus_cloud::local_state::CloudLocalStore::default()
        .load_device()
        .ok()
        .flatten()
    ...
    {
        Some(device_id) => builder.header("X-CodexPlus-Device-Id", device_id),
        None => builder,
    }
}

fn with_codexplus_device_headers_if(
    builder: reqwest::RequestBuilder,
    enabled: bool,
) -> reqwest::RequestBuilder {
    if enabled {
        with_codexplus_device_headers(builder)
    } else {
        builder
    }
}
...
pub async fn handle_responses_proxy_request(body: &str) -> anyhow::Result<ProxyHttpResponse> {
    ...
    if upstream.body_kind == UpstreamProxyBodyKind::Responses {
        return Ok(ProxyHttpResponse {
            status: http_status_line(status_code),
