...
#[tokio::test]
async fn launch_starts_helper_for_codexplus_cloud_provider() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    ...
    let settings = BackendSettings {
        ...
        relay_profiles: vec![RelayProfile {
            ...
            name: "Codex++ Cloud".to_string(),
            base_url: "https://codex-plus.example/v1".to_string(),
            upstream_base_url: "https://codex-plus.example/v1".to_string(),
            api_key: "sk-test".to_string(),
            protocol: RelayProtocol::Responses,
            relay_mode: codex_plus_core::settings::RelayMode::PureApi,
            ..RelayProfile::default()
        }],
        active_relay_id: MANAGED_PROVIDER_ID.to_string(),
        ..BackendSettings::default()
    };
    let hooks = FakeHooks::new(events.clone()).with_settings(settings);
