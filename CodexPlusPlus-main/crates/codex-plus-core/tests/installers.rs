...
#[test]
fn windows_entrypoint_plan_contains_internal_and_visible_entrypoints() {
    ...
    assert!(plan.silent_shortcut.ends_with("Codex++ 静默启动.lnk"));
...
#[test]
fn macos_bundle_metadata_contains_internal_and_visible_apps() {
    let options = InstallOptions {
    ...
    assert!(silent.app_path.ends_with("Codex++ 静默启动.app"));
    assert!(manager.app_path.ends_with("Codex++.app"));
    assert!(silent.info_plist.contains("<string>Codex++ 静默启动</string>"));
    assert!(manager.info_plist.contains("<string>Codex++</string>"));
...
fn installer_exports_expected_two_entrypoint_names() {
    assert_eq!(shortcut_names(), ("Codex++ 静默启动.lnk", "Codex++.lnk"));
    assert_eq!(app_bundle_names(), ("Codex++ 静默启动.app", "Codex++.app"));
}

#[test]
fn companion_binary_path_resolves_macos_silent_app_next_to_manager_app() {
    let manager_exe = std::path::Path::new(
        "/Applications/Codex++.app/Contents/MacOS/CodexPlusPlusManager",
    );
    ...
    assert_eq!(
        companion,
        std::path::PathBuf::from("/Applications/Codex++ 静默启动.app/Contents/MacOS/CodexPlusPlus")
    );
    assert_ne!(
        companion,
        std::path::PathBuf::from(
            "/Applications/Codex++.app/Contents/MacOS/codex-plus-plus"
        )
    );
}
...
fn macos_bundle_does_not_wrap_the_bundle_executable_in_itself() {
    let options = InstallOptions {
        install_root: Some("/Applications".into()),
        launcher_path: Some("/Applications/Codex++ 静默启动.app/Contents/MacOS/CodexPlusPlus".into()),
        manager_path: Some("/Applications/Codex++.app/Contents/MacOS/CodexPlusPlusManager".into()),
        remove_owned_data: false,
    };
