...
type Route = "cloud" | "overview" | "relay" | "sessions" | "context" | "enhance" | "zedRemote" | "userScripts" | "recommendations" | "maintenance" | "about" | "settings";
...
const routes: Array<{ id: Route; label: string; icon: LucideIcon }> = [
  ...
  { id: "relay", label: "高级供应商", icon: KeyRound },
...
class DesktopRuntimeUnavailableError extends Error {
  constructor() {
    super("desktop-runtime-unavailable");
  }
}
...
function isDesktopRuntimeUnavailable(error: unknown): boolean {
  return error instanceof DesktopRuntimeUnavailableError;
}
...
export function App() {
  ...
  const call = <T,>(command: string, args?: Record<string, unknown>) => {
    if (!hasDesktopRuntime()) return Promise.reject(new DesktopRuntimeUnavailableError());
  ...
  const navigate = async (next: Route) => {
    setRoute(next);
    window.localStorage.setItem("codex-plus-route", next);
    if (next === "cloud") {
...
function routeSubtitle(route: Route) {
  const subtitles: Record<Route, string> = {
    cloud: "登录、权益、安装、用量和诊断",
    overview: "检查问题、启动与快速修复",
    relay: "管理高级供应商、协议、Key 与配置文件",
...
function loadInitialRoute(): Route {
  ...
  const saved = window.localStorage.getItem("codex-plus-route");
  if (saved && routes.some((item) => item.id === saved)) return saved as Route;
  return "cloud";
}
